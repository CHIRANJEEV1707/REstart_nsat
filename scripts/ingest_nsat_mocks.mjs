...
// Hardcode URI if env fails (User provided context implies local/dev env)
...
async function ingestMocks() {
    try {
        ...
        // Adjust paths for backend execution context
        const projectRoot = path.resolve(__dirname, '..'); // Assuming script is in backend/scripts or similar
        // actually we are running from backend dir, so project root is one up
                ...
                if (m1) {
                    const transformed = {
                        ...
                        questions: m1.sections.flatMap(s => s.questions.map(q => ({
                            ...
                            correctAnswer: (q.options.findIndex(opt => opt === q.correct_answer) + 1).toString(),
                            topics: [q.topic],
                            marks: 4,
                            negativeMarks: 1
                        })))
