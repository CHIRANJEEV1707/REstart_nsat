...
const seedData = async () => {
    try {
        ...
        const createdExams = await Exam_1.default.insertMany(examsData);
        ...
        for (let i = 1; i <= 50; i++) {
            ...
            if (type === "Public") {
                ...
                if (Math.random() > 0.7)
                    requiredExams.push("JEE Advanced");
            }
            else {
        ...
        await College_1.default.insertMany(colleges);
        ...
        const hashedPassword = await bcryptjs_1.default.hash('password123', salt); // Default password for all
        const usersData = [];
